  ...
  const rows=items.map((i,idx)=>{
    ...
    const catCells=REPORT_CATS.map(rc=>{
      ...
      if (isNAStatus(st)) {
        ...
        stMark = `<span class="vmx-st" data-tt="${hesc(st)}">●</span>`;
      } else {
...
function openProjectPage(projectName, fromHistory=false){
  ...
  initRatings();
  const a=all();
  const items=a.filter(i=>(i.jobDesc||i.jobCode||i.sheet||'')===projectName);
  ...
  setM(`
    ...
    <div class="shd">
      <h2 class="stitle">Project Dashboard</h2>
...
// Body layout follows the L&T Construction – Renewables vendor-rating letter format:
...
function emailVendor(code){
  ...
  // "LE/LE24M782/POD/25/000013" -> "LE24M782...013"
  const shortPO=po=>{
    const s=String(po||'');
    const parts=s.split('/');
  ...
  const head=
    ...
    `Greetings from L&T Construction \u2013 Renewables.\n\n`+
...
// VIEW: VENDOR SCORECARD (L&T Matrix - Multi-Page)
